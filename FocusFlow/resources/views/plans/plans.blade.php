...
@section('content')
    ...
    <div class="page-header mb-4 border-radius-xl">
        ...
        <div class="container">
            ...
            <a href="/goal-plans" type="button" class="btn bg-green">{{__('Plan for your goals')}}</a>
        ...
        <div class="col-md-12 mt-4">
            <div class="card bg-gray-800">
                <div class="card-header pb-0 px-3 bg-gray-800">
                    <h6 class="mb-0 text-white">{{__('Your Plans')}}</h6>
                </div>
                        ...
                        @foreach($plans as $plan)
                            ...
                            <li class="list-group-item border-0 d-flex p-4 mb-2 mt-3 bg-gray-700 border-radius-lg">
                                <div class="d-flex flex-column">
                                    <h6 class="mb-3 text-green ">{{$plan->title}}</h6>

                                    <span class="mb-2 "><h6 class="text-white opacity-7">{{__('Goal Name:')}}  @if(isset($goals[$plan->goal_id]))
                                    ...
                                    <span class="text-xs text-white opacity-7">{{__('Date:')}} <span class="text-white opacity-8 ms-sm-2 font-weight-bold">{{$plan->date->format(config('app.date_format'))}}</span></span>
                                </div>
                                <div class="ms-auto text-end">
                                    <a class="btn btn-link text-white px-3 mb-0" href="/view-plan?id={{$plan->id}}"><i class="fas fa-file-alt text-green me-2" aria-hidden="true"></i>{{__('View')}}</a>


                                    <a class="btn btn-link text-white px-3 mb-0" href="/goal-plans?id={{$plan->id}}"><i class="fas fa-pencil-alt text-green me-2" aria-hidden="true"></i>   {{__('Edit')}}
                                        </a>
